...
export default {
  ...
  data() {
    const checkUsername = (rule, value, callback) => {
      if (!value) return callback(new Error('Please input an username'))
    ...
    const validatePass = (rule, value, callback) => {
      if (value === '') callback(new Error('Please input the password'))
        ...
        if (this.ruleForm.checkPass !== '') this.$refs.ruleForm.validateField('checkPass')
    ...
    const validatePass2 = (rule, value, callback) => {
      ...
      else if (value !== this.ruleForm.pass) callback(new Error("Two inputs don't match!"))
      else callback()
    }
    return {
      ruleForm: {
