import { Tag, Task, TagExtended } from '../models'
...
class ReportPlugin {
  ...
  public generateReportData(tasks: Task[]) {
    ...
    for (const task of tasks)
      for (const tag of task.tags) {
        ...
        if (tagsMap.has(key)) tagEx.seconds += tagsMap.get(key)!.seconds
  ...
  public mapFirstTagsWithOther(number: number, sortedMap: Map<string, TagExtended>) {
    ...
    for (const [, value] of sortedMap.entries()) {
      if (index >= number) {
        if (returnValue.has(other)) tagExtendedOther.seconds += value.seconds
